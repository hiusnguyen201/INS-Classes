...
│   └── SecurityConfig.java      # Filter chain (stateless, /auth/** + /public/** open, rest authenticated), CORS (allowedOriginPatterns=*, all methods/headers, allowCredentials), JSON 401 entry point, PasswordEncoder bean
├── security/                # JWT infrastructure
│   ├── JwtService.java          # Generate/parse HS256 tokens (claims: sub=userId, email, type)
│   └── JwtAuthFilter.java       # Populates SecurityContext from Bearer token; catches BusinessException (TOKEN_EXPIRED / TOKEN_INVALID) and writes JSON 401 directly to response
...
└── service/                 # Business logic (one service per module)
    ├── EmailService.java        # @Async HTML email via Gmail SMTP; sendUserPasswordEmail() uses ClassPathResource template at resources/templates/email/user-password.html
...
| GET | `/users` | List users — `Pageable` + optional `?keyword=` (name/email LIKE) + `?type=` filter; always sorted `createdAt DESC` |
| GET | `/users/{id}` | Get user by id |
| PUT | `/users/{id}` | Update user (partial: name, avatar, type) |
| GET | `/users/check-email` | Check if email exists (`?email=xxx`) → `Boolean` |
...
- `JwtAuthFilter` catches `BusinessException` and writes a JSON 401 (`TOKEN_EXPIRED` / `TOKEN_INVALID`) directly to the response — expired/invalid tokens no longer surface as 500
...
- `CreateUserInput.isSendPasswordViaEmail` — implemented: sends HTML email asynchronously via `EmailService` (`@Async`); requires `MAIL_USERNAME` + `MAIL_PASSWORD` env vars set to a valid Gmail app password; email failure is logged and does not fail the request
